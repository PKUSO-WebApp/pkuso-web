 ...
 * 2. 管理面板：只读详情 + 底部操作行（编辑/删除双按钮，右下角规范，Issue #182）。
 ...
 *      真实失败时列表保留，用户可重试）。
 *    无锁定/解锁入口（Issue #212）：非 admin 修改 is_locked 被 DBA 触发器拒绝，
 *    锁定/解锁仅 admin 端可用（admin/community 操作行），成员面板只读展示 🔒 徽章。
 * 3. 列表/面板数据派生自同一份 usePosts.data：update/remove 的乐观更新自动同步两处视图
 *    （面板帖子对象由列表派生，删除后面板自动回列表）。
 * 4. busy（删除 pending）：操作行按钮互斥禁用 + 弹窗关闭守卫（防慢网下误操作）。
 */
type Props = {
  /** 关闭弹窗（返回「我的」页） */
  onClose: () => void;
...
export function PublishedPostsModal({ onClose }: Props) {
  ...
  // 删除后列表乐观更新，派生自动失效（删除后面板自动回列表，双保险见 handleDelete）
  ...
  // busy（删除 pending）期间禁止关闭弹窗（遮罩关闭禁用 + 关闭按钮守卫），
        ...
        >
          {selected ? (
            ...
            <div className="max-h-[90vh] space-y-3 overflow-y-auto text-xs">
              ...
              {/* 底部操作行：编辑 / 删除（busy 期间互相禁用；Issue #182：右下角；
                  Issue #212：无锁定入口——非 admin 改 is_locked 被 DBA 触发器拒绝） */}
