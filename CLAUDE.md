...
渐进式多分支重构,详见 `.claude/REFACTORING_ROADMAP.md`。目录结构文档待重构定型后补充。
...
- **组件复用**: 写新 UI 前先查 `src/components/ui/`(Modal/Toggle/Card)和 `src/components/rehearsal/`。Button 暂不统一(20+ 变体,待设计系统定型)。
