...
/**
 * 彻底删除一个测试用户：调用 auth.admin.deleteUser。
 *
 * profiles_id_auth_fkey 设置了 ON DELETE CASCADE，删除 auth.users 行会
 * 级联删除 profiles 及其所有子表（schedules、posts、attendances 等），
 * 无需手动清理。
 *
...
export async function deleteTestUser(_client: SupabaseClient, userId: string): Promise<void> {
  ...
  // 污染（其 REST 请求携带用户 JWT → RLS 静默拒删 → deleteUser 撞 FK 23503）
...
/**
 * 预清扫：删除历史残留的全部测试账号（auth.users + profiles 级联）。
 * 在 beforeAll / 测试开头最先调用，保证测试开始前环境干净，
 * 同时自动清掉历史 CI 崩溃留下的孤儿账号。
 *
 * profiles_id_auth_fkey ON DELETE CASCADE：删除 auth.users 行会级联删除
 * profiles 及其所有子表，无需手动清理 profiles。
 */
...
export async function sweepTestUsers(_client: SupabaseClient): Promise<void> {
  // 独立 service role client，防调用方 client 的登录 session 污染
  const clean = await createCleanAdminClient();
  const cutoffMs = Date.now() - SWEEP_MIN_AGE_MS;

  // 逐个删除匹配的 auth.users（admin API 不支持按邮箱 like 批量删）。
  // listUsers 默认每页 50 条，历史残留可能超过一页（生产库曾积压 42 个），
  // 循环翻页直到取完，避免漏删。
  // listUsers 返回的 user.created_at 现成可用，只清理 10 分钟前的。
  let page = 1;
  const perPage = 200;
  while (true) {
